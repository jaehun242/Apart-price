...
test('full official transaction-price series preserves raw and rebases precisely', () => { const d = buildDataset(rows('seoul'), rows('busan')); assert.equal(d.data.length, 139); assert.equal(d.data[0].seoul, 100); assert.equal(d.data[0].busan, 100); assert.equal(d.data[0].seoul_raw, 66); assert.equal(d.statbl_id,'A_2024_00178'); assert.equal(d.category,'공동주택 실거래가격지수'); assert.equal(d.housing_type,'아파트'); assert.equal(d.transaction_type,'매매'); assert.equal(d.regions.seoul.id,'500007'); assert.equal(d.regions.busan.id,'500008'); });
