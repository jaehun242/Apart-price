...
test('secret normalization safely handles whitespace, quotes and one encoded layer',()=>{assert.equal(normalizeSecret('  "abc%2Bdef%2Fghi%3D"  '),'abc+def/ghi=');assert.equal(normalizeSecret('plain-key'),'plain-key');});
